...
and original Manim scenes/media in animations, docs/media/unknown-space.*,
and docs/media/footprint-radius.*.
...
- ReplicaCAD: pinned LICENSE.txt says CC BY-NC 4.0; website says CC BY 4.0.
  Conflict unresolved. Dataset and derived scene assets are not distributed here.
- Depth Anything 3: selected pinned code and DA3METRIC-LARGE weights are recorded
  as Apache-2.0; other model variants must not be assumed to share these terms.
  Source/weights are acquired separately with recorded revisions and checksums.
- Runtime dependencies retain their own licenses. docs/dependency-provenance.json
  records installed metadata and license texts; it does not override upstream terms.
